...
def clearName(name):
    return name.replace(COLON, SPACE).replace(INTERROGATION, NOTHING)\
           .replace(SLASH, NOTHING).replace(QUOTES, NOTHING)\
           .replace(LESSTHAN, NOTHING).replace(GREATERTHAN, NOTHING)\
...
def myfunction():
    ...
    for line in lines:
        if THREE_DASHES in line:
            ...
            i += 1
            folder_name = clearName(str(i).zfill(leading_zeroes)+ ' -- ' + line[line.index(THREE_DASHES) + len(THREE_DASHES) :-5])
    ...
    for line in lines:
        if THREE_DASHES not in line:
            if line.strip() and FOLDER_INDEX_SEPARATOR in line:
                print('what this ' + clearName(line.strip()).split(' ', 1)[1].split(FOLDER_INDEX_SEPARATOR)[0].split(' ',1)[1])
                files_in_folder[line.split(SPACE)[1]] = [clearName(line.strip()).split(' ', 1)[1].split(FOLDER_INDEX_SEPARATOR)[0].split(' ',1)[1]]
    ...
    for f in files_list:
        ...
        print(os.getcwd())
        new_file_name = os.path.join(tut_folder.strip(), files_in_folder[idx][0] + EXTENSION)
        print(new_file_name)
